...
train_li9, test_li9, train_li9lab, test_li9lab = 
			train_test_split(li9, li9_label, random_state=None)
...
clf.fit(train_li9, train_li9lab)
...
li9_label.index = range(len(li9_label))

#get the labels for the test data
li9_test_labels = li9_label.iloc[li9_rows,:]
...
test_li9.to_csv('li9_classifiedtestdata.csv')

#save the model
filename = 'li9_classifier.sav'
...
train_n17, test_n17, train_n17lab, test_n17lab = 
			train_test_split(n17, n17_label, random_state=None)
...
n17_label.index = range(len(n17_label))
n17_test_labels = n17_label.iloc[n17_rows,:]
test_n17.loc[:,'label'] = n17_test_labels
test_n17.loc[:,'scores'] = n17_score
test_n17.to_csv('n17_classifiedtestdata.csv')
filename = 'n17_classifier.sav'
...
train_neu, test_neu, train_neulab, test_neulab =
                        train_test_split(neu, neu_label, random_state=None)
...
neu_label.index = range(len(neu_label))
neu_test_labels = neu_label.iloc[neu_rows,:]
...
train_world, test_world, train_worldlab, test_worldlab =
                        train_test_split(world, world_label, random_state=None)
...
world_label.index = range(len(world_label))
world_test_labels = world_label.iloc[world_rows,:]
test_world.loc[:,'label'] = world_test_labels
test_world.loc[:,'scores'] = world_score
test_world.to_csv('world_classifiedtestdata.csv')
filename = 'world_classifier.sav'
...
train_geo, test_geo, train_geolab, test_geolab =
                        train_test_split(geo, geo_label, random_state=None)
...
geo_prob = gep_clf.predict_proba(test_geo)
...
test_gep.loc[:,'classifier'] = geo_predictions
geo_rows = test_geo.index
geo_label.index = range(len(geo_label))
geo_test_labels = geo_label.iloc[geo_rows,:]
test_geo.loc[:,'label'] = geo_test_labels
test_geo.loc[:,'scores'] = geo_score
test_geo.to_csv('geo_classifiedtestdata.csv')
filename = 'geo_classifier.sav'
joblib.dump(geo_clf, filename)
#create an roc curve for the signal classifications by each model
li9_signal =li9_ prob[:,1]
